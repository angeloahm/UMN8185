! This code solves a simple value function iteration
...
program  vfi
    ...
    integer :: i_k, i_k_prime, iter       !Indexes and iteration stuff
    ...
    !Define grids
	real, dimension(n_k) :: k_grid
    real, dimension(n_k) :: k_policy
    real, dimension(n_k) :: c_policy
    integer, dimension(n_k) :: policy_index
    real, dimension(n_k) :: valuefunction
	real, dimension(n_k) :: V0=0.0
    real, dimension(n_k) :: V_old=0.0
    real, dimension(n_k) :: V_temp=0.0
    ...
    do i_k = 1, n_k
        k_grid(i_k) = 0.75*k_ss + ((i_k-1)/(n_k-1))*k_ss
    end do
    ...
    do iter = 1, max_iter
        ...
        !Howard's algorithm should come here

        !Check convergence
        norm = maxval(abs(valuefunction(:) - V0(:)))
